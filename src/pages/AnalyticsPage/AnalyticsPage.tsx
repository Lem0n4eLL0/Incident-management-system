...
const CARTS_OPTIONS = ['Линейный', 'По типу', 'По подразделениям'] as const;
export const COLORS = [
  'var(--charts-100)',
  'var(--charts-200)',
  'var(--charts-300)',
...
export const transformByType: IncidentDataTransformer<{ name: string; value: number }> = (
  ...
  //Кол-во инцидентов по типам (PieChart)
...
export const transformByDate: IncidentDataTransformer<{ date: string; count: number }> = (
  incidents
) => {
  //  Инциденты по дням
  const result: Record<string, number> = {};
...
export const AnalyticsPage = () => {
  const [currentChart, setCurrentChart] = useState<CratsOptions>('Линейный');
  ...
  const dataDaysIncidentChart = useMemo(() => transformByDate(incidents), [incidents]);
    ...
    <div className={style.content}>
      <section className={style.charts}>
        <div className={style.controls}>
          <div className={style.charts_controller}>
            <h2 className={style.select_title}>Выберите срез</h2>
            <Select<CratsOptions>
              options={['Линейный', 'По типу', 'По подразделениям']}
          ...
          <button className={clsx(formStyle.confirm_button)} onClick={() => setIsModalOpen(true)}>
            Сформировать отчет
          </button>
        </div>
        <div className={style.chart}>
          ...
          {currentChart === 'Линейный' && (
            <>
              <h3 className={style.chart_title}>По дате происшествия</h3>
          ...
          {currentChart === 'По подразделениям' && (
            <>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  layout="vertical"
                  data={dataByUnitChart}
                  margin={{ top: 20, right: 20, left: 100, bottom: 20 }}
                >
                  <XAxis type="number" allowDecimals={false} />
                  <YAxis type="category" dataKey="unit" width={150} tick={{ fontSize: 14 }} />
                  <Tooltip />
